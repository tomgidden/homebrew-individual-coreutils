#!/usr/bin/env bash
# Primary build path: fetch the official coreutils release tarball for
# whatever version Homebrew's own coreutils.rb currently pins, verify
# its GPG signature against the known maintainer key, extract,
# configure, and build the utilities in utils.txt.
#
# Deliberately NOT "latest GNU release" (contrast individual-coreutils,
# which does track latest) -- this repo exists to mirror what Homebrew
# actually ships, so a user gets the same coreutils version whether
# they came from homebrew/core's own coreutils or this tap.
#
...
source "$SCRIPT_DIR/resolve-homebrew-version.sh"

BUILD_DIR="${ROOT_DIR}/.build-tarball"
OUT_DIR="${ROOT_DIR}/out"

echo "==> Resolving coreutils version pinned by Homebrew's formula"
CU_VERSION="$(resolve_homebrew_coreutils_version)"
TARBALL="coreutils-${CU_VERSION}.tar.xz"
echo "==> Homebrew-pinned version: $CU_VERSION"
