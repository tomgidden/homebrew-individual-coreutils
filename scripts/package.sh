#!/usr/bin/env bash
# Package each built utility in out/<name>/ into a release archive for
# this repo's own GitHub Releases -- just bin + man page + LICENSE.
# Unlike individual-coreutils' packages, these are only ever consumed
# by a Homebrew formula's `install` block, not run by hand, so there's
# no install.sh or per-package README to include.
#
...
for util_dir in "$OUT_DIR"/*/; do
  ...
  archive="$DIST_DIR/homebrew-individual-coreutils-${u}-${CU_VERSION}-${ARCH}.tar.gz"
